...
import type { TreeNode, FileNode, FolderNode } from "../../utils/generate-tree";
...
export function SidebarProvider({ children }: { children: ReactNode }) {
    ...
    return (
        <SidebarContext.Provider value={{ open, setOpenSearch }}>
            <button
                className={clsx(
                    "w-full h-10 bg-background border-b-[1px] px-8",
                    "flex flex-row gap-2 items-center text-sm",
                    "sticky inset-x-0 top-[56.8px] z-50 lg:hidden"
                )}
